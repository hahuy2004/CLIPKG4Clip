...
class RawVideoExtractorCV2():
    """Original CLIPKG4Clip video extractor using OpenCV."""
...
class RawVideoExtractorCV2_TempMe():
    """TempMe video extractor with advanced data augmentation using OpenCV."""
    def __init__(self, centercrop=False, size=224, framerate=-1, subset="test"):
        if not TEMPME_MODULES_AVAILABLE:
            raise ImportError(
                "TempMe modules not available. Please ensure video_transforms and random_erasing are present."
            )
...
def RawVideoExtractor(use_tempme=False, **kwargs):
    """
    Factory function to create the appropriate video extractor.
    
    Args:
        use_tempme (bool): If True, use TempMe extractor with data augmentation.
                          If False, use original CLIPKG4Clip extractor.
        **kwargs: Arguments passed to the extractor constructor.
        
    Returns:
        RawVideoExtractorCV2 or RawVideoExtractorCV2_TempMe instance.
        
    Example:
        # Original extractor
        extractor = RawVideoExtractor(use_tempme=False, size=224, framerate=1)
        
        # TempMe extractor with augmentation
        extractor = RawVideoExtractor(use_tempme=True, size=224, framerate=1, subset="train")
    """
    if use_tempme:
        return RawVideoExtractorCV2_TempMe(**kwargs)
    else:
        return RawVideoExtractorCV2(**kwargs)